...
OLD_TDB=$(curl https://api.github.com/repos/TrinityCore/TrinityCore/releases | jq 'map(select(.tag_name|startswith("TDB335"))) | sort_by(.created_at) | reverse | .[0]')
...
wget $OLD_TDB_URL -O TDB.7z
7z e TDB.7z

#  8. set new TDB name and sql name
NEW_TDB_VERSION=`date +%y%m0`
TODAY=`date +%Y_%m_%d`
...
echo $NEW_RELEASE_RESPONSE | jq
curl -s -H "Authorization: token $GITHUB_TOKEN" -H "Content-Type: application/octet-stream" $(echo $NEW_RELEASE_RESPONSE | jq -r '.upload_url' | sed -e 's${?name,label}$$g')'?name='"$NEW_TDB_FILE"'.7z'  --data-binary @$NEW_TDB_FILE.7z | jq
curl -s -H "Authorization: token $GITHUB_TOKEN" -X 'PATCH' $(echo $NEW_RELEASE_RESPONSE | jq -r '.url') -d '{"draft":false}' | jq
